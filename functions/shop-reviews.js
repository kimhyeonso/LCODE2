...
exports.recordShopOrder = onCall(options, async ({ auth, data }) => {
  ...
  const items = data.items.map((item) => {
    if (!id(item.id) || typeof item.name !== "string" || item.name.length > 200) throw new HttpsError("invalid-argument", "상품 정보를 확인해 주세요.");
...
exports.saveProductReview = onCall(options, async ({ auth, data }) => {
  ...
  if (!Array.isArray(photos) || photos.length > 3 || photos.some((photo) => typeof photo.name !== "string" || photo.name.length > 255 || typeof photo.src !== "string" || photo.src.length > 180000 || !/^data:image\/jpeg;base64,[A-Za-z0-9+/=]+$/.test(photo.src))) throw new HttpsError("invalid-argument", "사진 정보를 확인해 주세요.");
  ...
  return db.runTransaction(async (tx) => {
    ...
    if (!old) {
      const orders = (await tx.get(db.doc(`users/${auth.uid}/shop/purchaseHistory`))).data()?.orders || [];
      const now = Date.now();
      const order = orders.find((order) => order.userId === auth.uid && !["cancelled", "refunded"].includes(order.status) && Date.parse(order.orderedAt) <= now && Date.parse(order.orderedAt) >= now - 30 * 86400000 && order.items?.some((item) => item.id === data.productId));
      if (!order) throw new HttpsError("failed-precondition", "최근 30일 이내의 주문 내역이 필요합니다.");
      productName = order.items.find((item) => item.id === data.productId).name;
